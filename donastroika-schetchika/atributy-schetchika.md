...
| user\_scope | Необязательный | Scope \(домен\), которым определяется идентификатор пользователя. Для одного пользователя можно задать разные scope, например, мобильный, десктопный и пр. Указывается в коде скрипта счетчика в качестве параметров атрибута options. Подробнее про передачу атрибута – см. «[Передача идентификатора пользователя](../razmetka-celevykh-deistvii/peredacha-identifikatora-polzovatelya.md)». |
